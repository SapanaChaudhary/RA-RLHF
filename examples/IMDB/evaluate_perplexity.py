...
def build_dataset(config, query_dataset, input_min_text_length=2, input_max_text_length=8, data_split = 'train'):
    ...
    ds = load_dataset(query_dataset, split=data_split+'[:200]')
...
#train_dataset = build_dataset(args.ppo_config, args.query_dataset, data_split='train')
...
sft_ppo_model = trl_model_class.from_pretrained(
    '/mnt/shared-scratch/Anon1/auth1/rlhf/trl/logs/sft_ppo/2023-09-26_10-36-41/model.pt',
    trust_remote_code=True,
    device_map=device_map,
    peft_config=peft_config,
)
...
testers_all = [zero_shot_tester, sft_tester, ppo_tester, sft_ppo_tester] 
...
for epoch, batch in tqdm(enumerate(ppo_tester.dataloader)):
    ...
    for ii, tester in enumerate(testers_all):
        ...
        print(f'model: {model_names_all[ii]}\n\n input:{batch["query"][:5]}, output:{batch["response"][:5]} \n\n')
        ...
        #pdb.set_trace()
        if ii == 0:
            perplexity_scores = perplexity.compute(predictions=texts, model_id='gpt2')['perplexities']
        elif ii == 1:
            perplexity_scores = perplexity.compute(predictions=texts, model_id='lvwerra/gpt2-imdb')['perplexities']
        elif ii == 2:
            perplexity_scores = perplexity.compute(predictions=texts, local_file_only='/mnt/shared-scratch/Anon1/auth1/rlhf/trl/logs/ppo/2023-09-26_11-27-23/pytorch_model.bin')['perplexities']
